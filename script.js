...
document.addEventListener('click',e=>{if(e.target&&e.target.id==='callPopup')closeCallPopup();if(e.target&&e.target.id==='whatsappPopup')closeWhatsappPopup();});document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeCallPopup();closeWhatsappPopup();}});
function copyIban(){navigator.clipboard.writeText('IT65Z0200861624000107426851');alert('IBAN copiato / IBAN е копиран');}
